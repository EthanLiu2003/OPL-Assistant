...
const BASE_URL = 'https://powerlifting-america.com/events/list/';
const MAX_PAGES = 20;

const buildPageUrl = (page: number): string =>
  page === 1 ? BASE_URL : `${BASE_URL}page/${page}/`;

export const fetchPaMeets = async (): Promise<Meet[]> => {
  const all: Meet[] = [];
  ...
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`PA events page ${page} ${resp.status}`);
    }
    const html = await resp.text();
    if (hasNoResultsIndicator(html)) break;

    const pageMeets = parseTribeEventsHtml(html, { source: 'pa', federation: 'PA' });
